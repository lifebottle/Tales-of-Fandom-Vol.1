...
class Pak:
    ...
    def get_pak_type(data: bytes) -> int | None:
        ...
        # Check for alignment
        if first_entry % 0x800 == 0:
            is_aligned = True

        if pak1_header_size % 0x800 != 0:
            pak1_check = pak1_header_size + (0x800 - (pak1_header_size % 0x800))
        else:
        ...
        if data_size > pak3_header_size:
            calculated_size = 0
            for size in struct.unpack(f"<{files}I", data[4 : (files + 1) * 4]):
                calculated_size += size
            if calculated_size == data_size - pak3_header_size:
                return 0
